...
The compensatory model makes a specific quantitative prediction: the net spaceflight effect under each light condition should be smaller than the main condition effect estimated from the additive model. We confirmed this by comparing the interaction model condition coefficients with the additive model condition coefficients, finding that the interaction model estimates were systematically shrunk toward zero for genes with large light effects, consistent with the compensatory interpretation. The additive model itself identified a substantially larger, light-averaged spaceflight footprint (2,132 leaf and 2,582 root DEGs) — dominated by oxidative-stress response and phenylpropanoid metabolism and localised to root ground-tissue and vascular cell types — providing the "core footprint" against which these light-dependent interaction effects are interpreted (Supplementary Note 1, Supplementary Fig. 20–21, Supplementary Data 14).
